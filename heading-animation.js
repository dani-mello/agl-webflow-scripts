console.log("STAGGERED HEADING (LINES) ANIMATION LOADED v6 (no flash)");

(function () {
  const prefersReduced =
    window.matchMedia &&
    window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  if (prefersReduced) return;

  if (
    typeof gsap === "undefined" ||
    typeof ScrollTrigger === "undefined" ||
    typeof SplitText === "undefined"
  ) {
    console.warn("GSAP, ScrollTrigger or SplitText missing");
    return;
  }

  gsap.registerPlugin(ScrollTrigger, SplitText);

  const headings = gsap.utils.toArray(".u-animate-heading");
  if (!headings.length) return;

  headings.forEach((heading) => {
    if (heading.dataset.animateInit === "1") return;
    heading.dataset.animateInit = "1";

    // hide until split is applied (prevents first-load flash)
    gsap.set(heading, { autoAlpha: 0 });

    const split = new SplitText(heading, {
      type: "lines",
      linesClass: "u-split-line"
    });

    // now reveal the element (still animated via lines)
    gsap.set(heading, { autoAlpha: 1 });

    gsap.from(split.lines, {
      yPercent: 120,
      x: -18,
      rotate: 1,
      opacity: 0,
      duration: 1.1,
      ease: "power3.out",
      stagger: 0.3,
      scrollTrigger: {
        trigger: heading,
        start: "top 85%",
        toggleActions: "play none none none",
        invalidateOnRefresh: true
      }
    });
  });
})();
